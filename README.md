...
- If you use the account ID and access token from the Instagram Use Cases section of the Meta developer portal, leave Graph API Host blank or set it to `instagram`. LynxPoster auto-detects common Instagram-login tokens and uses `graph.instagram.com`. Use `facebook` only for Facebook/Page access-token setups that require `graph.facebook.com`.
...
- After every destination for a post has published successfully, LynxPoster removes that post's local media attachments immediately.
...
- Instagram giveaway private scans are controlled by `INSTAGRAM_PRIVATE_SCAN_MODE`, which defaults to `manual_only`. In that default mode, the 5-minute autorun cycle and automatic giveaway close use only official Graph/webhook data; follow checks and public-profile repost checks remain provisional/manual review until you intentionally run a private scan.
- Set `INSTAGRAM_PRIVATE_SCAN_MODE=end_only` only if you want closing a giveaway to perform one private verification scan. Set `INSTAGRAM_PRIVATE_SCAN_MODE=weekly` only if you accept the extra account risk of scheduled private scans; `INSTAGRAM_PRIVATE_SCAN_INTERVAL_HOURS` then rate-limits those scans and defaults to `168`.
- The optional Private Verification Session ID or username/password fields are used only for explicit giveaway private scans, risk-enabled close/weekly scans, and the login diagnostic button. Avoid using them when an Instagram account has automation warnings or lockouts.
